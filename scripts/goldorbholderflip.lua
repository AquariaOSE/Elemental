...
function update(me, dt)
	if entity_getState(me)==STATE_IDLE then
		if energyOrb == 0 then
			orb = entity_getNearestEntity(me, "goldorbflip")
			if orb ~=0 then
			...
			if not openedDoors and entity_isState(goldorbflip, STATE_CHARGED) then
				openedDoors = true
				node = entity_getNearestNode(me)
				node_activate(node)
			end
			if not savedOrb and entity_isState(goldorbflip, STATE_IDLE) then
				node = entity_getNearestNode(me)
				node_activate(node)
				savedOrb = true
			end
			if openedDoors and entity_isState(goldorbflip, STATE_IDLE) then
				openedDoors = false
			end
		end
	end
