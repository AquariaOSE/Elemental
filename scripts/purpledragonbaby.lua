...
-- ================================================================================================
-- P U R P L E D R A G O N B A B Y    (BASED ON EVILJELLY)
-- ================================================================================================
...
function init(me)
	setupBasicEntity(
	me,
	"purpledragonbaby/baby-head",						-- texture
	5,							-- health
	2,							-- manaballamount
	2,							-- exp
	10,							-- money
	24,							-- collideRadius (for hitting entities + spells)
    ...
    entity_initHair(me, 40, 2, 20, "purpledragonbaby/baby-body")
    ...
    entity_setEatType(me, EAT_FILE, "Raspberry") -- well, have nothing better...
...
function update(me, dt)
    ...
    if v.avoidTimer > 0 then
        avoid = true
        v.avoidTimer = v.avoidTimer - dt
    end
    ...
    if v.shotT >= 0 and entity_isEntityInRange(me, v.n, 600) then
        v.shotT = v.shotT - dt
        if v.shotT <= 0 then
            v.shotT = math.random(666, 1350) / 1000
            local s = createShot("purpleseadragon-mini", me)
            shot_setAimVector(s, entity_getAimVector(me, 0, 1))
        end
    end
