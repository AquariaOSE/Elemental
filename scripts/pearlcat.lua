...
-- oldhealth : 40
	...
	entity_initHair(me, 160, 4, 64, "pearlcat")
	--[[
	entity_initSegments(
	25,								-- num segments
	2,								-- minDist
...
local function doSpawn(me)
    if chance(80) then
        if chance(70) then
            ...
            spawnIngredient("IceCream", entity_x(me), entity_y(me))
        end
    else
...
function update(me, dt)
	...
	if v.songTimer > 6 then
		...
		if v.spawns > 0 then
			playSfx("secret")
			doSpawn(me)
            entity_scale(me, 0.5, 0.5) -- hrm, does not work (entity is hair only)
			v.spawns = v.spawns - 1
		end
	end
