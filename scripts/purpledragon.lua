...
-- ================================================================================================
-- P U R P L E D R A G O N    (based on greenseadragon)
-- ================================================================================================
...
function init(me)
	...
	entity_initSkeletal(me, "purpledragon")	
	...
	entity_setDeathParticleEffect(me, "PurpleSeaDragonExplode")
...
function dieNormal(me)
	...
	if chance(5) then
		spawnIngredient("", entity_getPosition(me))
	else
		if chance(10) then
			spawnIngredient("", entity_getPosition(me))
		else
...
function update(me, dt)
	...
	elseif entity_isState(me, STATE_ATTACK) then
		...
		if v.fireDelay < 0 then
			...
			local s = createShot("PurpleSeaDragon", me, entity_getTarget(me), x, y)
	...
	if avatar_isBursting() and bone == v.bone_body and avatar_isLockable() and entity_setBoneLock(v.n, me, bone) then
	else
		entity_touchAvatarDamage(me, entity_getCollideRadius(me), 0, 1000)
	end
...
function enterState(me, state)
	...
	elseif entity_isState(me, STATE_DEAD) then
		shakeCamera(10, 3)
		createEntity("purpledragonbaby", "", entity_x(me)-10, entity_y(me))
		createEntity("purpledragonbaby", "", entity_x(me)+10, entity_y(me))
		createEntity("purpledragonbaby", "", entity_x(me), entity_y(me)-10)
		createEntity("purpledragonbaby", "", entity_x(me), entity_y(me)+10)
		createEntity("purpledragonbaby", "", entity_x(me)-20, entity_y(me))
		createEntity("purpledragonbaby", "", entity_x(me)+20, entity_y(me))
		createEntity("purpledragonbaby", "", entity_x(me), entity_y(me)-20)
		createEntity("purpledragonbaby", "", entity_x(me), entity_y(me)+20)
        
		local maxa = 3.14 * 2
		local a = 0
		while a < maxa do
			local s = createShot("PurpleSeaDragon-nosound", me)
            local vx, vy = math.sin(a), math.cos(a)
			shot_setAimVector(s, vx, vy)
            shot_setOut(s, 100)
			a = a + maxa/16.0
		end
...
function damage(me, attacker, bone, damageType, dmg)
    if attacker ~= 0 and (entity_isName(attacker, "purpledragon") or entity_isName(attacker, "purpledragonbaby")) then
        return false
    end
